...
dataPath = './individual_stocks_15m_60D/'

def download_stock(stock):
	""" try to query the iex for a stock, if failed note with print """
	try:
		TICK = yf.Ticker(stock)
		stock_df = TICK.history(period='60d', interval='15m') 
...
if __name__ == '__main__':
	...
	s_and_p = [
		# adding world indices 
		'^DJI', '^NDX', '^GDAXI', 'FTSE100', '^HSI', '^N225', '^NYA', '^SSEC',
		# adding SP500 
	...
	if len(bad_names) > 0:
		with open('failed_queries_60m_2Y.txt','w') as outfile:
